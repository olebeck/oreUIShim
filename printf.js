 ...
 * @argument str {String}
 * @argument re {RegExp}
 * @argument parseDelim {Function}
 * @argument instance {Object}
*/
function tokenize(str, re, parseDelim, instance){
  // summary:
  ...
  while((match = re.exec(str))){
    const content = str.slice(lastIndex, re.lastIndex - match[0].length);
    if(content.length){
      tokens.push(content);
    }
    if(parseDelim){
...
class Formatter {
    ...
    _parseDelim(mapping, intmapping, flags, minWidth, period, precision, specifier) {
        if(mapping){
          this._mapped = true;
        }
        return {
    ...
    _specifiers = {
        ...
        for(let i = 0; i < this._tokens.length; i++){
          ...
          if(typeof token == 'string'){
            ...
            if(!token.compiled){
              ...
              if(mixins.extend){
                const s = this._specifiers[mixins.extend];
                for(const k in s){
                  mixins[k] = s[k];
                }
      ...
      formatObject = function(token) {
        // If no precision is specified, then reset it to null (infinite depth).
        const precision = (token.period === '.') ? token.precision : null;
      ...
      zeroPad = function(token, /*Int*/ length) {
        ...
        const tenless = length - 10;
        while(token.arg.length < tenless){
          token.arg = (token.rightJustify) ? token.arg + this._zeros10 : this._zeros10 + token.arg;
        }
        const pad = length - token.arg.length;
...
function sprintf() {
    ...
    const string = formatter.format(...args);
    return string;
}
